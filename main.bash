...
stateful_workload_stop() {
  ...
  kubectl annotate cluster -n awx awx --overwrite cnpg.io/hibernation=on
  kubectl annotate cluster -n quay clair --overwrite cnpg.io/hibernation=on
  kubectl annotate cluster -n finance-tracker finance --overwrite cnpg.io/hibernation=on
  kubectl annotate cluster -n gitea gitea --overwrite cnpg.io/hibernation=on
  kubectl annotate cluster -n grafana grafana --overwrite cnpg.io/hibernation=on
  kubectl annotate cluster -n homeassistant homeassistant --overwrite cnpg.io/hibernation=on
  kubectl annotate cluster -n immich immich --overwrite cnpg.io/hibernation=on
  kubectl annotate cluster -n netbox netbox --overwrite cnpg.io/hibernation=on
  kubectl annotate cluster -n nextcloud-fpm nextcloud --overwrite cnpg.io/hibernation=on
  kubectl annotate cluster -n quay quay --overwrite cnpg.io/hibernation=on
  kubectl annotate cluster -n analytics-for-spotify spotify --overwrite cnpg.io/hibernation=on
  kubectl annotate cluster -n analytics-for-spotify-ext spotify --overwrite cnpg.io/hibernation=on
  kubectl annotate cluster -n stackrox stackrox --overwrite cnpg.io/hibernation=on
  kubectl annotate cluster -n zitadel zitadel --overwrite cnpg.io/hibernation=on
...
stateful_workload_start_pre() {
  ...
  kubectl annotate cluster -n awx awx --overwrite cnpg.io/hibernation=off
  kubectl annotate cluster -n quay clair --overwrite cnpg.io/hibernation=off
  kubectl annotate cluster -n finance-tracker finance --overwrite cnpg.io/hibernation=off
  kubectl annotate cluster -n gitea gitea --overwrite cnpg.io/hibernation=off
  kubectl annotate cluster -n grafana grafana --overwrite cnpg.io/hibernation=off
  kubectl annotate cluster -n homeassistant homeassistant --overwrite cnpg.io/hibernation=off
  kubectl annotate cluster -n immich immich --overwrite cnpg.io/hibernation=off
  kubectl annotate cluster -n netbox netbox --overwrite cnpg.io/hibernation=off
  kubectl annotate cluster -n nextcloud-fpm nextcloud --overwrite cnpg.io/hibernation=off
  kubectl annotate cluster -n quay quay --overwrite cnpg.io/hibernation=off
  kubectl annotate cluster -n analytics-for-spotify spotify --overwrite cnpg.io/hibernation=off
  kubectl annotate cluster -n analytics-for-spotify-ext spotify --overwrite cnpg.io/hibernation=off
  kubectl annotate cluster -n stackrox stackrox --overwrite cnpg.io/hibernation=off
  kubectl annotate cluster -n zitadel zitadel --overwrite cnpg.io/hibernation=off
...
install_addons() {
  ...
  local lh_count=0
  lh_count=$(kubectl get pods -n longhorn-system | grep -cv Running) || { echo "Failed to get Longhorn pods count" >&2; return 1; }
  while [[ "${lh_count}" -ne 1 ]]; do
    sleep 1
  done
...
get_dashboard_secret() {
  ...
  local sa_name=""
  sa_name=$(kubectl get serviceaccount admin-user -n kubernetes-dashboard -o jsonpath="{.secrets[0].name}") || { echo "Failed to get service account secret name" >&2; return 1; }
...
preseed_server() {
  ...
  local preseed_hash=""
  preseed_hash=$(mkpasswd -s -m md5 "${PASSWORD}") || { echo "Failed to generate password hash" >&2; return 1; }
  sed -i "s,#d-i passwd/user-password-crypted password <HASH>,d-i passwd/user-password-crypted password ${preseed_hash},g" "/tmp/preseed/preseed.cfg"
...
install_cluster() {
  ...
  for NODE in ${NODES}; do
    ...
    while [[ "${finished}" = "0" ]]; do
      sleep 5
      local running=0
      running=$(virsh list --all | grep "running" | grep "${PREFIX}${NODE}" | wc -c) || { echo "Failed to check VM ${NODE} status" >&2; return 1; }
...
install_k3s_cluster() {
  ...
  local node_count=0
  node_count=$(kubectl get nodes | wc -l) || { echo "Failed to get node count" >&2; return 1; }
  local expected=0
  expected=$(echo "${NODES}" | wc -w) || { echo "Failed to count expected nodes" >&2; return 1; }
  while [[ "${node_count}" -le "${expected}" ]]; do
    sleep 1
  done
...
install_okd_prep() {
  ...
  sudo setquota -u "$(whoami)" 0 0 0 0 /tmp || true
  ...
  if [[ -z "${VAULT_TOKEN}"  ]]; then
    export VAULT_TOKEN
    VAULT_TOKEN=$(vault login --tls-skip-verify -address="${URL}" -method=userpass -token-only username=arthur) || { echo "Failed to login to vault" >&2; return 1; } || { echo "Failed to login to vault" >&2; return 1; } || { echo "Failed to login to vault" >&2; return 1; }
  fi
  ...
  local pull_secret=""
  pull_secret=$(printf '%s\n' "${PULL_SECRET}" | sed 's/[&\/]/\&/g') || { echo "Failed to escape pull secret" >&2; return 1; }
  sed -i "s/<PULL_SECRET>/\x27${pull_secret}/g" "${OKD}/okd/install-config.yaml"
...
install_okd() {
  ...
  local kadmin_pass=""
  kadmin_pass=$(cat "${OKD}/okd/auth/kubeadmin-password") || { echo "Failed to read kubeadmin password" >&2; return 1; }
  echo "Kubeadmin Password: ${kadmin_pass}"
...
install_addons_okd_virt() {
  ...
  if [[ -z "${VAULT_TOKEN}"  ]]; then
    export VAULT_TOKEN
    VAULT_TOKEN=$(vault login --tls-skip-verify -address="${URL}" -method=userpass -token-only username=arthur) || { echo "Failed to login to vault" >&2; return 1; }
  fi
  ...
  local mp_count=0
  mp_count=$(kubectl get pods -n openshift-marketplace | grep -v "Completed" | grep -cv Running) || { echo "Failed to get Marketplace pods count" >&2; return 1; }
  while [[ "${mp_count}" -ne 1 ]]; do
    sleep 1
  done
...
install_addons_okd() {
  ...
  echo -e "\n${BLUE}Waiting for Longhorn to Boot:${NC}"
  local lh_count2=0
  lh_count2=$(kubectl get pods -n longhorn-system | grep -cv Running) || { echo "Failed to get Longhorn pods count" >&2; return 1; }
  while [[ "${lh_count2}" -ne 1 ]]; do
    sleep 1
  done
  ...
  while [[ ${_om} -lt 5 ]]; do
    local _okm
    _okm=$(kubectl kustomize "${HOMELAB}"/okd/openshift-monitoring/overlays/sandbox | argocd-vault-plugin generate - | kubectl apply -f -) || { echo "Failed to apply monitoring" >&2; return 1; }
    if [[ "${_okm}" == "0" ]]; then
      break
    fi
    _om=$((_om + 1))
