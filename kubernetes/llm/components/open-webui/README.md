...
Open WebUI uses two storage backends:

- **S3 (Rook-Ceph RGW)** — uploaded documents, user avatars, and other file
  uploads are stored in a Ceph object store bucket provisioned via
  [ObjectBucketClaim](object-bucket-claim.yaml). The bucket name is
  auto-generated by Rook (e.g. `open-webui-<hash>`). See the
  [S3 storage docs](https://docs.openwebui.com/tutorials/maintenance/s3-storage/)
  for details on the Open WebUI S3 integration.

- **Local PVC** (`open-webui-data`, `ReadWriteMany`, `rook-cephfs`, 5Gi —
  see [pvc.yaml](pvc.yaml)) — PostgreSQL database files, Chroma vector
  embeddings, Tiktoken/cache data, and the upload staging directory
  (`DATA_DIR/uploads/`) still reside on the PVC. Open WebUI writes uploads
  locally first, then copies them to S3.
