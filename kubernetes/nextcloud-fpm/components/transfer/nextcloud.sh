...
cp /mnt/nextcloud/rclone.conf /tmp/.config/rclone/rclone.conf
...
LIST_FILE="${1:-/mnt/nextcloud/nextcloud-users.txt}"

RCLONE_BASE_OPTS=( sync --ignore-existing --stats=10s --fast-list --stats-log-level NOTICE \
  --multi-thread-streams 8 --drive-chunk-size 128M --max-backlog 999999 \
  --transfers=25 --checkers=100 --buffer-size=75M )
